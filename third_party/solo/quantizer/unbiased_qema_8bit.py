...
class UnbiasedQemaOptimState8bit(TorchAOBaseTensor):
...
if not torch_version_at_least("2.4.0"):
    ...
    def _to(self, *args, **kwargs):
        ...
        return UnbiasedQemaOptimState8bit(
    ...
    UnbiasedQemaOptimState8bit.to = _to
    del _to  # make sure to not re-use


@UnbiasedQemaOptimState8bit.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, UnbiasedQemaOptimState8bit) and isinstance(src, UnbiasedQemaOptimState8bit):
        assert (
    ...
    elif isinstance(dst, UnbiasedQemaOptimState8bit):
...
@UnbiasedQemaOptimState8bit.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
    device = kwargs.get("device", None)
    out = UnbiasedQemaOptimState8bit(
...
@UnbiasedQemaOptimState8bit.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, UnbiasedQemaOptimState8bit) else x for x in args]
    return func(*args, **kwargs)


# this is needed for DTensor.from_local() and for flattening tensor
@UnbiasedQemaOptimState8bit.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args

    if tuple(x.shape) == tuple(shape):
        return UnbiasedQemaOptimState8bit(x.codes, x.scale, x.alpha, x.signed, x._shape, x.quantile)

    if len(shape) == 1 and shape[0] == -1:
        return UnbiasedQemaOptimState8bit(x.codes, x.scale, x.alpha, x.signed, (x.numel(),), x.quantile)
...
@UnbiasedQemaOptimState8bit.implements(
    [
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, UnbiasedQemaOptimState8bit):
        raise ValueError(f"expecting a UnbiasedQemaOptimState8bit but found {type(x)}")
...
@UnbiasedQemaOptimState8bit.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
    return (
...
@UnbiasedQemaOptimState8bit.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
