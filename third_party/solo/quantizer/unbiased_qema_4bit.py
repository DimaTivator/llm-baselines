...
class UnbiasedQemaOptimState4bit(TorchAOBaseTensor):
...
if not torch_version_at_least("2.4.0"):
    ...
    def _to(self, *args, **kwargs):
        ...
        return UnbiasedQemaOptimState4bit(
    ...
    UnbiasedQemaOptimState4bit.to = _to
    del _to  # make sure to not re-use


@UnbiasedQemaOptimState4bit.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, UnbiasedQemaOptimState4bit) and isinstance(src, UnbiasedQemaOptimState4bit):
        assert (
    ...
    elif isinstance(dst, UnbiasedQemaOptimState4bit):
...
@UnbiasedQemaOptimState4bit.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
    device = kwargs.get("device", None)
    out = UnbiasedQemaOptimState4bit(
...
@UnbiasedQemaOptimState4bit.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, UnbiasedQemaOptimState4bit) else x for x in args]
    return func(*args, **kwargs)


# this is needed for DTensor.from_local() and for flattening tensor
@UnbiasedQemaOptimState4bit.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args

    if tuple(x.shape) == tuple(shape):
        return UnbiasedQemaOptimState4bit(x.codes, x.scale, x.alpha, x.signed, x._shape, x.quantile)

    if len(shape) == 1 and shape[0] == -1:
        return UnbiasedQemaOptimState4bit(x.codes, x.scale, x.alpha, x.signed, (x.numel(),), x.quantile)
...
@UnbiasedQemaOptimState4bit.implements(
    [
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, UnbiasedQemaOptimState4bit):
        raise ValueError(f"expecting a UnbiasedQemaOptimState4bit but found {type(x)}")
    ...
    return UnbiasedQemaOptimState4bit(codes, scale, alpha, x.signed, shape, x.quantile)


# required by torch.distributed.checkpoint.save
# note that we don't actually implement pin memory for this tensor subclass
# (pin_memory argument is ignored in aten._to_copy)
@UnbiasedQemaOptimState4bit.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
    return (
...
@UnbiasedQemaOptimState4bit.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
...
if torch_version_at_least("2.5.0"):
    ...
    add_safe_globals([UnbiasedQemaOptimState4bit])
