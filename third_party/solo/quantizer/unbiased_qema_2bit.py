...
class UnbiasedQemaOptimState2bit(TorchAOBaseTensor):
...
if not torch_version_at_least("2.4.0"):
    ...
    def _to(self, *args, **kwargs):
        ...
        return UnbiasedQemaOptimState2bit(
    ...
    UnbiasedQemaOptimState2bit.to = _to
    del _to  # make sure to not re-use


@UnbiasedQemaOptimState2bit.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, UnbiasedQemaOptimState2bit) and isinstance(src, UnbiasedQemaOptimState2bit):
        assert (
    ...
    elif isinstance(dst, UnbiasedQemaOptimState2bit):
...
@UnbiasedQemaOptimState2bit.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
    device = kwargs.get("device", None)
    out = UnbiasedQemaOptimState2bit(
...
@UnbiasedQemaOptimState2bit.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, UnbiasedQemaOptimState2bit) else x for x in args]
    return func(*args, **kwargs)


# this is needed for DTensor.from_local() and for flattening tensor
@UnbiasedQemaOptimState2bit.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args

    if tuple(x.shape) == tuple(shape):
        return UnbiasedQemaOptimState2bit(x.codes, x.scale, x.alpha, x.signed, x._shape, x.quantile)

    if len(shape) == 1 and shape[0] == -1:
        return UnbiasedQemaOptimState2bit(x.codes, x.scale, x.alpha, x.signed, (x.numel(),), x.quantile)
...
@UnbiasedQemaOptimState2bit.implements(
    [
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, UnbiasedQemaOptimState2bit):
        raise ValueError(f"expecting a UnbiasedQemaOptimState2bit but found {type(x)}")
...
@UnbiasedQemaOptimState2bit.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
    return (
...
@UnbiasedQemaOptimState2bit.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
