...
def unbiased_quantize_with_alpha(input: Tensor, alpha: Tensor, bits: int = 4):
    shape = input.shape
    logalpha = alpha.log2().view(-1, 1)
    input = input.view(logalpha.size(0), -1)
    codes_down = input.log2().div_(logalpha).floor_().clip_(0, 2 ** bits - 1)
    codes_up = (codes_down + 1).clip_(0, 2 ** bits - 1)
    val_up = alpha.view(-1, 1) ** codes_down
    val_down = alpha.view(-1, 1) ** codes_up
    residual = input - val_down
    p = (val_up - val_down).mul_(
        torch.rand(residual.size(), generator=adaq_generator, device=input.device)
    )
    codes = torch.where(
        residual >= p, codes_down, codes_up
    )
    return codes.view(shape).to(torch.uint8)
