...
# ─── SlimAdam-specific ───────────────────────────────────────────────────────
# Leave blank to use the bundled Llama layer map.
# Set to a path to override with a custom layer map or explicit rules JSON.
SLIM_ADAM_LAYER_MAP=""   # e.g. "src/optim/memory_efficient/slim_adam/layer_map.json"
SLIM_ADAM_RULES_JSON=""  # e.g. "my_rules.json"
...
for BETA2 in "${BETA2_LIST[@]}"; do
    ...
    EXP_NAME="llama257M_slim_adam_${DTYPE}_lr${LR}_wd${WD}_b2${BETA2}_fineweb"
    echo "==============================================================="
    echo "Starting: ${EXP_NAME}"
    echo "==============================================================="

    # Build optional SlimAdam flags
    SLIM_ARGS=""
