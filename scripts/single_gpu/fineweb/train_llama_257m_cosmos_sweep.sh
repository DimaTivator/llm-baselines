...
# ─── COSMOS-specific ─────────────────────────────────────────────────────────
DENSITY=0.25          # rank = density * p.size(1) per parameter
COSMOS_LR_RATIO=0.1
COSMOS_GAMMA=0.2
...
for BETA2 in "${BETA2_LIST[@]}"; do
    ...
    EXP_NAME="llama257M_cosmos_${DTYPE}_lr${LR}_wd${WD}_b2${BETA2}_fineweb"
    echo "==============================================================="
    ...
    torchrun --standalone --nproc_per_node="${NGPUS}" src/main.py \
        ...
        \
        --density ${DENSITY} \
        --cosmos_lr_ratio ${COSMOS_LR_RATIO} \
        --cosmos_gamma ${COSMOS_GAMMA} \
        --cosmos_nestrov \
        \
        --scheduler cos \
        --warmup-steps ${WARMUP} \
