...
# ─── Adam-mini-specific ──────────────────────────────────────────────────────
# n_kv_heads=0 means standard MHA (n_kv_heads == n_heads)
ADAM_MINI_N_KV_HEADS=0
...
for BETA2 in "${BETA2_LIST[@]}"; do
    ...
    EXP_NAME="llama257M_adam_mini_${DTYPE}_lr${LR}_wd${WD}_b2${BETA2}_fineweb"
    echo "==============================================================="
    ...
    torchrun --standalone --nproc_per_node="${NGPUS}" src/main.py \
        ...
        --grad-clip 1.0 \
        \
        --adam_mini_n_kv_heads ${ADAM_MINI_N_KV_HEADS} \
        \
        --scheduler cos \
        --warmup-steps ${WARMUP} \
