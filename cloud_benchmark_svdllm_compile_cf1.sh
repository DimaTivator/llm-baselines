...
OUTPUT_PATH="${RESULT_DIR}/results-${COMPILE_MODE}.json"
...
if [[ -f "${OUTPUT_PATH}" ]]; then
    python - "${OUTPUT_PATH}" "${HF_REPO_ID}" "${COMPILE_MODE}" \
        >>"${LOG_PATH}" 2>&1 <<'PY'
import sys

from huggingface_hub import HfApi

result_path, repo_id, compile_mode = sys.argv[1:]
path_in_repo = f"results/cloud-h100-compile-cf1-{compile_mode}.json"
HfApi().upload_file(
