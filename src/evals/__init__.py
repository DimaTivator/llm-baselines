...
def build_evaluators(cfg, tokenizer=None):
    ...
    needs_tokenizer = DownstreamEvaluator.is_enabled(cfg) or AuxiliaryLMEvaluator.is_enabled(cfg)
    ...
    if AuxiliaryLMEvaluator.is_enabled(cfg):
        lm_evaluator = AuxiliaryLMEvaluator(cfg=cfg, tokenizer=tokenizer)
