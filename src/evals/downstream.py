...
def _select_latest_metrics(raw_metrics):
    ...
    # We keep only *_v2 to reduce logging noise.
...
def _disable_metric_distributed_sync(metric):
    # Downstream eval runs on rank 0 only, so torchmetrics must not sync across processes.
...
class DownstreamEvaluator:
    ...
    def is_enabled(cfg):
        return bool(
            getattr(cfg, "downstream_eval_enabled", False)
            and getattr(cfg, "downstream_eval_interval", 0) > 0
        )
