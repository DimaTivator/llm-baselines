"""Aggregate the ViT-on-CIFAR10 (14.2M-param) noise_frac x spectral_l1_reg_coef
sweep (scripts/vit_label_noise/train_noisefrac_sweep.sh) into a summary table
and a plot mirroring mnist_label_noise/analyze_noisefrac_sweep.py.

Usage: python src/vit_label_noise/analyze_noisefrac_sweep.py
"""
import json
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "exps" / "vit_label_noise"
OUT_PNG = Path(__file__).resolve().parents[2] / "spectral-wd-gen-vit-noisefrac-scan.png"
...
def plot(df: pd.DataFrame) -> None:
    ...
    axes[0].set_title("ViT-CIFAR10 (14.2M params): test acc vs coef")
    axes[0].legend()
