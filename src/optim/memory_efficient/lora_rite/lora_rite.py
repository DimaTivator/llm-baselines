...
class _LoraRiteHelper:
    ...
    def inf_to_nan(self, array):
        ...
        return torch.nan_to_num(
            array, nan=torch.nan, posinf=torch.nan, neginf=torch.nan
        )
    ...
    def inverse_sqrt(
        self, x, esc, epsilon, epsilon_root, relative_epsilon=True, force_positive=False
    ):
        if relative_epsilon:
    ...
    def get_unmagnified_rotate_second_escape(self, v_new, v_old):
        ...
        eigen_diff = torch.maximum(
            torch.max(eig_old - eig_new), torch.tensor(0).to(eig_old)
        )
        trace_diff = torch.maximum(
            torch.trace(v_old) - torch.trace(v_new), torch.tensor(0).to(eig_old)
        )
    ...
    def clip_update(self, update, clip_threshold: float):
        ...
        denom = torch.maximum(
            torch.ones_like(mean_update), mean_update / clip_threshold
        )
        return update / denom
...
class LoRARite(Optimizer):
    def __init__(
        ...
        if not 0.0 <= beta1 < 1.0:
            raise ValueError(
                "Invalid beta parameter: {} - should be in [0.0, 1.0[".format(beta1)
            )
        if not 0.0 <= beta2 < 1.0:
            raise ValueError(
                "Invalid beta parameter: {} - should be in [0.0, 1.0[".format(beta2)
            )
        if not 0.0 <= epsilon:
            raise ValueError(
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p1, p2 in list(zip(group["params"], group["params"][1:]))[::2]:
                ...
                update_l, update_r = helper.inf_to_nan(p1.grad.data), helper.inf_to_nan(
                    p2.grad.data
                )
