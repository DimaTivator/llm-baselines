            ...
            for p in group["params"]:
                ...
                if "rank" in group:
                    if "projector" not in state:
                        state["projector"] = GradientProjector(group["rank"], update_proj_gap=group["update_proj_gap"], alpha=group["alpha"], proj_type=group["proj_side"], proj_alg=group["proj_type"])
                    grad = state["projector"].project(grad, state["step"])

                # State initialization
                if "exp_avg" not in state or ("update_proj_gap" in group and state["step"] % group["update_proj_gap"] == 0 and group.get("reset_statistics", False)):
