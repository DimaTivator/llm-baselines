...
class COSMOS(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 lr_ratio=0.1, density=0.25, weight_decay=0, gamma=0.2,
        ...
        if not 0.0 < density <= 1.0:
            raise ValueError(f"Invalid density value: {density} - should be in (0.0, 1.0]")
                ...
                if p.grad is not None:
                    ...
                    if len(state) == 0:
                        ...
                        if len(p.size()) == 2 and p.size(0) <= 10000:
                            rank = max(1, int(self.density * p.size(1)))
                            state['exp_avg_GG'] = torch.zeros(rank, rank, dtype=p.data.dtype, device=p.data.device)
                            state['exp_avg_P'] = torch.zeros(p.size(1), rank, dtype=p.data.dtype, device=p.data.device)
                            state['exp_avg_sq'] = torch.zeros(p.size(0), rank, dtype=p.data.dtype, device=p.data.device)
                        else:
