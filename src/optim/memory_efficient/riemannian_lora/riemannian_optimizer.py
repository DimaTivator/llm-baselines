...
class FixedRank:
    ...
    def embedding(self, point: list, vec: list, ambient: bool = True):
        U_l, _ = point
...
class RiemannianLora(Optimizer):
    ...
    def apply_second_momentum(self, point: list, grad_vec: list, momentum_vec: list, state: dict, beta: list, manifold: FixedRank):
        ...
        if 'second_momentum' not in state:
            ...
            addm = [1 / (torch.sqrt(state['second_momentum'][0]) + 1e-8),
                    1 / (torch.sqrt(state['second_momentum'][1]) + 1e-8)]
            return momentum_vec[0] * addm[0], momentum_vec[1] * addm[1]

        state['second_momentum'] = [
            beta[1] * state['second_momentum'][0] + (1 - beta[1]) * norm[0],
            beta[1] * state['second_momentum'][1] + (1 - beta[1]) * norm[1],
        ]
        addm = [1 / (torch.sqrt(state['second_momentum'][0]) + 1e-8),
                1 / (torch.sqrt(state['second_momentum'][1]) + 1e-8)]
        return momentum_vec[0] * addm[0], momentum_vec[1] * addm[1]
    ...
    @torch.no_grad()
    def get_deltas(self, A, B, manifold: FixedRank):
        dot_A = A.grad.T  # (m, 2r)
        dot_B = B.grad.T  # (2r, n)
        ...
        vec = manifold.euclidian_to_reimanian(point=point, vec=[dot_A, dot_B])
        return point, vec
    ...
    def apply_momentum(self, point: list, vec: list, state: dict, manifold: FixedRank, beta: float) -> list:
        ...
        m_vec = manifold.transport(
            point_X=state['prev_point'],
            point_Y=point,
            vec_X=state['momentum'],
        )
...
class RiemannianSGD(RiemannianLora):
    def __init__(self, params, **defaults):
        ...
        super().__init__(params, **{'lr': lr, 'betas': [momentum, 0.0]})

    @torch.no_grad()
    def apply_second_momentum(self, point, grad_vec, momentum_vec, state, beta, manifold):
        return momentum_vec


class RiemannianPretrainOptimizer(Optimizer):
    """Compound optimizer: RiemannianLora/SGD for LoRA factors + AdamW for regular params."""

    def __init__(self, lora_groups, regular_groups, lr, betas, weight_decay, eps, opt_type="adamw"):
        if opt_type == "sgd":
            self._lora_opt = RiemannianSGD(lora_groups, lr=lr, momentum=betas[0])
        else:
            self._lora_opt = RiemannianLora(lora_groups, lr=lr, betas=list(betas))

        self._regular_opt = torch.optim.AdamW(
            regular_groups, lr=lr, betas=betas, weight_decay=weight_decay, eps=eps,
        )

        # Bypass Optimizer.__init__ (it rejects empty param lists) and set the
        # required attributes manually — same pattern used by LoRAOptimizer in
        # this codebase.  Inheriting from Optimizer is enough for isinstance checks.
        torch._C._log_api_usage_once(f"python.optimizer.{self.__class__.__name__}")
        self.defaults = {'lr': lr}
        self.state = {}

        # Combined param_groups: scheduler sets lr on these dicts directly, which
        # propagates to both sub-optimizers because they share the same dict objects.
        self.param_groups = self._lora_opt.param_groups + self._regular_opt.param_groups

    def step(self, closure=None):
        self._lora_opt.step()
        self._regular_opt.step()

    def zero_grad(self, set_to_none: bool = True):
        self._lora_opt.zero_grad(set_to_none=set_to_none)
        self._regular_opt.zero_grad(set_to_none=set_to_none)

    def state_dict(self):
        return {
            'lora_opt': self._lora_opt.state_dict(),
            'regular_opt': self._regular_opt.state_dict(),
        }

    def load_state_dict(self, state_dict):
        self._lora_opt.load_state_dict(state_dict['lora_opt'])
        self._regular_opt.load_state_dict(state_dict['regular_opt'])
        self.param_groups = self._lora_opt.param_groups + self._regular_opt.param_groups
