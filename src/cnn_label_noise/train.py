"""Label-noise generalization study on CIFAR10 with a small CNN, mirroring
vit_label_noise/train.py (same warmup+cosine schedule and init-fix pattern).
Reuses AdamWSpectralL1Reg and models/compress.py, both extended to handle
Conv2d weights (viewed as the 2D "filter matrix" [out_ch, in_ch*kh*kw]) --
see optim/adamw_spectral_L1_reg.py and models/compress.py for that extension.

The classifier head is deliberately kept in a *separate* optimizer param group
with spectral_l1_reg_coef=0 (never regularized), based on the small-head
rank-collapse failure mode found in the time-series studies (§7/§8 of the
Obsidian report) -- excluding it here avoids repeating that failure mode.

Must be run with `src/` on sys.path, e.g. from the repo root:
    PYTHONPATH=./src python -m cnn_label_noise.train --noise_frac 0.25 --spectral_l1_reg_coef 1.0
See scripts/cnn_label_noise/train_noisefrac_sweep.sh for the full sweep.
"""
...
def train_one_run(cfg: CNNLabelNoiseConfig) -> dict:
    torch.manual_seed(cfg.seed)
    run_name = cfg.run_name or f"cnn_c{cfg.base_channels}_nf{cfg.noise_frac}_sl1_{cfg.spectral_l1_reg_coef}_seed{cfg.seed}"
    ...
    model = get_model("cnn", cfg).to(cfg.device)
    n_params = sum(p.numel() for p in model.parameters())
    logger.info("run=%s params=%.2fM", run_name, n_params / 1e6)

    if cfg.include_head_in_reg:
        ...
        other_params = [p for p in model.parameters() if id(p) not in head_param_ids]
        optimizer = AdamWSpectralL1Reg(
            [
                {"params": other_params, "spectral_l1_reg_coef": cfg.spectral_l1_reg_coef},
                {"params": head_params, "spectral_l1_reg_coef": 0.0},
            ],
            lr=cfg.lr,
            betas=(cfg.beta1, cfg.beta2),
            svt_interval=0,
    ...
    # torch.linalg.svd is not implemented on MPS (Apple Silicon) so that device
    # needs a CPU copy for this analysis; CUDA supports it natively, and this
    # model/dataset (CIFAR10-resolution conv net, full 10k-image test set,
    # evaluated once per RANK_GRID entry) is slow enough on CPU to matter.
    analysis_device = "cpu" if cfg.device == "mps" else cfg.device
    analysis_model = copy.deepcopy(model).to(analysis_device)
    ranks = model_effective_ranks(analysis_model, skip_names=())
    curve = truncation_sweep(analysis_model, test_loader, analysis_device)
    final = history[-1]
    result = {
        "run_name": run_name,
        "seed": cfg.seed,
...
def parse_args() -> CNNLabelNoiseConfig:
    p = argparse.ArgumentParser()
    defaults = dataclasses.asdict(CNNLabelNoiseConfig())
    for field in dataclasses.fields(CNNLabelNoiseConfig):
    ...
    args = p.parse_args()
    return CNNLabelNoiseConfig(**vars(args))
