"""Test LASER compression on a llama checkpoint.

Usage:
    python ./src/compression/test_laser.py \
...
def main():
    parser = argparse.ArgumentParser(description="Test LASER compression.")
    ...
    parser.add_argument(
        ...
        help="Module name suffixes to compress.",
    )
    ...
    # Apply LASER
    print(f"\nApplying LASER (rank={args.rank}) ...")
    apply_laser(model, rank=args.rank, target_modules=tuple(args.target_modules),
                device=args.device)
    model.eval()
    ...
    if args.wandb:
        ...
        wandb.init(
            ...
            name=f"{exp_name}_laser_rank{args.rank}",
            tags=["compress", "laser"],
