"""Test SVD-LLM compression on a llama checkpoint.

Usage:
    python ./src/compression/test_svd_llm.py \
...
def main():
    parser = argparse.ArgumentParser(description="Test SVD-LLM compression.")
    ...
    print(f"\nCollecting input covariance ({args.calib_batches} batches) ...")
    ...
    print(f"  Collected covariance for {len(whitening_stats)} layers.")

    # Apply SVD-LLM
    print(f"\nApplying SVD-LLM (rank={args.rank}) ...")
    apply_svd_llm(
        model, rank=args.rank, whitening_stats=whitening_stats,
    ...
    if args.wandb:
        ...
        wandb.init(
            ...
            name=f"{exp_name}_svd_llm_rank{args.rank}",
            tags=["compress", "svd_llm"],
