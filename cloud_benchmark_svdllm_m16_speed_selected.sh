...
EXPERIMENT_LIST=${EXPERIMENT_LIST:-src/compression/svdllm_m16_speed_checkpoints.txt}
...
LOG_PATH="${LOG_DIR}/benchmark-svdllm-m16-compile-b256-selected-$(date +%F_%H%M%S).log"
export TORCHINDUCTOR_CACHE_DIR=${TORCHINDUCTOR_CACHE_DIR:-/tmp/torchinductor-svdllm-m16-b256}
export TRITON_CACHE_DIR=${TRITON_CACHE_DIR:-/tmp/triton-svdllm-m16-b256}

mapfile -t EXPERIMENTS < <(sed '/^[[:space:]]*$/d' "${EXPERIMENT_LIST}")
if [[ "${#EXPERIMENTS[@]}" -ne 20 ]]; then
    echo "Expected 20 experiments, found ${#EXPERIMENTS[@]}"
    exit 0
fi
...
if [[ "${CHECK_ONLY}" == "1" ]]; then
    python -m py_compile \
        src/compression/benchmark_svd_llm_inference.py \
        src/compression/stage_svdllm_selected_cloud.py
...
(
    ...
    nvidia-smi --query-gpu=name,uuid,memory.total,memory.used,memory.free \
        --format=csv
...
if [[ -f "${OUTPUT_PATH}" && "${UPLOAD_RESULTS}" == "1" ]]; then
    python - "${OUTPUT_PATH}" "${GPU_PROCESS_LOG}" "${HF_REPO_ID}" <<'PY' \
        >>"${LOG_PATH}" 2>&1
import sys
...
for local_path, remote_path in (
    (result_path, "results/cloud-h100-m16-compile-b256-selected.json"),
    (process_path, "results/cloud-h100-m16-compile-b256-selected-gpu-processes.csv"),
):
    api.upload_file(
